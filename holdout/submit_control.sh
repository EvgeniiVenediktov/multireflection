...
#   RES=512 EPOCHS=<e> SBATCH_ARGS="--partition=h200 --cpus-per-task=32 --mem=160G" \
#       TRAIN_EXTRA="--compile --num-workers 24" bash holdout/submit_control.sh
set -euo pipefail
...
tid=$(sbatch --parsable --dependency=afterok:$jid --job-name=holdctltest-r$RES \
    --export=ALL,CKPT=$R/$jid/resnet18_l40s_${TAG}${jid}_last_model.pth,MODEL_RES=$RES,OUT_DIR=$R/$jid/holdout_test_last \
