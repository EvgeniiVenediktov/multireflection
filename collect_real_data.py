...
assert(isdir(DATA_COLLECTION_FOLDER))
...
def save_image(img: ArrayLike, x: float, y: float, folder: str = "data/new") -> None:
...
def whole_process(controller: MFController, x:float, y:float, pbar:tqdm, i, i_m, time_delay=0.1, log=True) -> None:
    ...
    # Save
    save_image(img, x, y, folder=DATA_COLLECTION_FOLDER)
